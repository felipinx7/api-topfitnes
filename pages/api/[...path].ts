...
export default async function handler(req, res) {
  ...
  console.log('Encaminhado para:', `http://72.60.8.246:4000${backendPath}`);
  console.log('---------------------\n');

  try {
    const response = await fetch(`http://72.60.8.246:4000${backendPath}`, {
      method: req.method,
      headers: safeHeaders,
